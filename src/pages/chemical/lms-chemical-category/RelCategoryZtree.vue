...
<template>
  <div>
    ...
    <ul id="storehouseTree" class="ztree tree_height"  v-show="isTree"  style="overflow-x: auto;" ></ul>
  </div>
</template>
    ...
    methods: {
      _Ztree() {
        ...
        this.$store.dispatch('LmsChemicalCategory/list').then(() => {
          ...
          treeObj = $.fn.zTree.init($("#storehouseTree"), setting, this.$store.state.LmsChemicalCategory.list);
          setTimeout(()=>{
