<template>
  <div>
    ...
    <ul id="roleTree" class="ztree tree_height" style="overflow-x: auto;" v-show="isTree"></ul>
  </div>
</template>
<script>
  /**
    ...
    methods: {
      _Ztree() {
        ...
        this.$store.dispatch('SysRole/roleTreeList').then(() => {
          treeObj=$.fn.zTree.init($("#roleTree"), setting, this.$store.state.SysRole.treeList);
          setTimeout(()=>{
      ...
      _search() {
        if(this.key !=''){
          ...
          this.$store.dispatch('SysRole/roleTreeKeyword',this.key).then(() => {
            treeObj = $.fn.zTree.init($("#roleTree"), setting, this.$store.state.SysRole.treeList);
            setTimeout(()=>{
              this.isloading = false;
              this.isTree = true;
