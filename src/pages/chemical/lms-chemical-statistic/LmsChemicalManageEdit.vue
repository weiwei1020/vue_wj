    ...
    <Modal v-model="showEditModal" :mask-closable="false" :width="900">
      ...
      <div>
        <Form id="edit-form-chemical-edit" ref="formObj" :model="formObj" :rules="ruleValidate" :label-width="85"
          ...
          <Form-item label="联系方式" prop="suppliertel" class="width-31-5">
          ...
          <Form-item label="email" prop="supplieremil" class="width-31-5">
            <autoComplete name="supplieremil"  placeholder="请输入供应商email" :value="formObj.supplieremail"
...
<script>
  ...
  export default {
    ...
    methods: {
      //从字典中查询类别
      _dicSearch() {
        this.$store.dispatch('LmsBaseDict/getItems', '制造厂商').then(() => {
          let list = [];
          let manufacturerList = this.$store.state.LmsBaseDict.item;
          for (let i = 0; i < manufacturerList.length; i++) {
            if(manufacturerList[i] != null) {
              list.push(manufacturerList[i].manufacturer);
            }
          }
          this.manufacturerList = list;
          this.manufacturerListTemp = list;
        });
        this.$store.dispatch('LmsBaseDict/getItems', '耗材名称').then(() => {
          let list = [];
          ...
          this.nameList = list;
          this.nameListTemp = list;
        });
        this.$store.dispatch('LmsBaseDict/getItems', '仓库位置').then(() => {
          let storehouseList = this.$store.state.LmsBaseDict.item;
          let list = [];
          for (let i = 0; i < storehouseList.length; i++) {
            if(storehouseList[i] != null) {
              list.push(storehouseList[i].storehouse)
            }
          }
          this.storehouseList = list;
          this.storehouseListTemp = list;
        });
        this.$store.dispatch('LmsBaseDict/getItems', '制造厂商').then(() => {
          let list = [];
          let manufacturerList = this.$store.state.LmsBaseDict.item;
        ...
        this.$store.dispatch('LmsBaseDict/getItems', '供应商').then(() => {
          let list = [];
          let supplierList = this.$store.state.LmsBaseDict.item;
          for (let i = 0; i < supplierList.length; i++) {
            list.push(supplierList[i].supplier);
          }
          this.supplierList = list;
          this.supplierListTemp = list;
        });
        //供应商联系方式
