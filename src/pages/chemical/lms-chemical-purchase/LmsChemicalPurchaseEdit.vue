  ...
  <div>
    <Modal v-model="showEditModal" :mask-closable="false" width="1100">
      ...
      <div slot="footer">
        ...
        <!--<Button @click="_ok(false)">保存</Button>-->
    ...
    <!--采购单进行编辑-->
    <RelLmsChemicalPurchaseEdit ref="relEditModel" @on-result-change="_search"></RelLmsChemicalPurchaseEdit>
  </div>
</template>
<script>
  /**
   * 添加编辑试验耗材申请表
   */
  import RelLmsChemicalPurchaseEdit from './LmsChemicalRelPurchaseEdit.vue'
  import BtnList from '../../../components/base/BtnList.vue'
  import PageTable from '../../../components/table/PageTable'

  export default {
    components: {
      RelLmsChemicalPurchaseEdit,
      BtnList,
      PageTable
    },
    data() {
      return {
        ...
        pageColumns: [
          ...
          {title: '耗材编号', key: 'chemicalNum', width: 120, ellipsis: true},
          {title: '耗材名称', key: 'chemicalName', width: 160, ellipsis: true},
          {
            title: '单价', key: 'price', width: 100, ellipsis: true,
            render: (h, data) => {
              return h("Input", {
                props: {
                  type: "number",
                ...
                on: {
                  'on-blur': (event,) => {
                    data.row.price = event.target.value;
                    this._cellChange(data.row._index, event.target.value, 'price');
                  },
                }
              })
            }
          ...
          {
            title: '采购数量', key: 'quantity', width: 120, ellipsis: true,
            render: (h, data) => {
              return h("Input", {
                props: {
                  type: "number",
                ...
                on: {
                  'on-blur': (event,) => {
                    data.row.quantity = event.target.value;
                    this._cellChange(data.row._index, event.target.value, 'quantity');
                  },
                }
              })
            }
...

          {
            ...
            render: (h, data) => {
              return h("Input", {
                props: {
                  type: "text",
                  value: data.row.supplier,
                },
                on: {
                  'on-blur': (event,) => {
                    data.row.supplier = event.target.value;
                    this._cellChange(data.row._index, event.target.value, 'supplier');
                  },
                }
              })
            }
          },
          {title: '备注', key: 'remark', width: 210, ellipsis: true,
            render: (h, data) => {
              return h("Input", {
                props: {
                  type: "text",
              ...
              let operate = [];
               let btnEdit =
                 h('Button', {
                   ...
                   on: {
                     click: () => {
                       this._edit(data.row.id, data.row.chemicalId);
                     }
                   }
                 }, '编辑');
              let btnDel =
    ...
    methods: {
      ...
      _blurSave(name, value, item) {
        ...
        this.$store.dispatch('LmsChemicalPurchase/editChemical', {id:item.id, obj:obj}).then(() => {
      ...
      _page() {
        this.$refs.pageTable._page('edit-form-detail', 'LmsChemicalPurchase/chemicalPageTemp');
      },
      _formSearch() {
      ...
      _ok(approve) {
        if (this.getPage.records.length == 0) {  //采购单不能为空
          ...
          if (approve) { //提交审批
            this.applyTitle = '采购单提交审批';
          } else {      //保存
            this.applyTitle = '采购单保存'
          }
        }
      },
      _cancel() {
      ...
      _purOrder(approve) {
        this.$store.dispatch('LmsChemicalPurchase/purOrder', {
          approve: approve,
          ...
          if (this.$store.state.LmsChemicalPurchase.success) {
            this.showEditModal = false;
            if (approve) { //提交审批
              this.$Message.success('采购单提交审批成功！');
            } else {     //保存
              this.$Message.success('采购单保存成功！');
            }
      ...
      _deletePurIds(ids, content) {
        this.$Modal.confirm({
          ...
          onOk: () => {
            this.$store.dispatch('LmsChemicalPurchase/deletePurIds', {ids: ids}).then(() => {
              if (this.$store.state.LmsChemicalPurchase.success) {
                this._page();
      ...
      _tableResultChange(msg, data) {
        switch (msg) {
          case 'page':
            this.getPage = this.$store.state.LmsChemicalPurchase.purchasePage;
            break;
          case 'selectIds':
            this.selectIds = data;
            break;
