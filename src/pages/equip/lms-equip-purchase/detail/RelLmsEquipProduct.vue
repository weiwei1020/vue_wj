<template>
  <div>
    <Modal v-model="showEquipModal" :mask-closable="false" :width="1200" class="modal-index-equip">
          ...
          <Collapse style="margin-bottom: 10px;" class="coll-search">
            <Panel>查询条件
              ...
              <Form id="equipInfo-search-form" inline onsubmit="return false" :label-width="90">
                <Form-item class="width-23" label="产品名称:" style="margin-bottom: 0">
                  <Input v-model="name" placeholder="请输入关键字" @on-enter="_search"></Input>
                </Form-item>
  ...
  export default {
    ...
    methods: {
      ...
      _searchParams() {
        ...
        return this.$extend(data, this.pageParams, {name: this.name});
      },
      _cancel() {
