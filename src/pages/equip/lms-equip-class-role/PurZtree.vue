    ...
    <div style="margin-top: 15px">
      <Form inline onsubmit="return false" v-show="isPur">
        <Form-item style="width:100%">
          <Input v-model="key" placeholder="请输入关键字，回车查询" @on-enter="_search" style="width:400px"
  ...
  export default {
    ...
    methods: {
      ...
      _getZtree() {
        ...
        this.$store.dispatch('LmsEquipClassRole/treeListAll').then(() => {
          $.fn.zTree.init($("#purTree"), setting, this.$store.state.LmsEquipClassRole.treeList);
      ...
      _ok() {
        ...
        this.$store.dispatch('LmsEquipClassRole/roleClass', this.idObj).then(() => {
          if (this.$store.state.LmsEquipClassRole.success) {
      ...
      _getselPurIds(ID) {  //回显选中的节点树
        var PURIDS = [];
        this.$store.dispatch('LmsEquipClassRole/excludeRoleList', ID).then(() => {
          var purIds = this.$store.state.LmsEquipClassRole.include;
      ...
      _search() {
        if (this.key != '') {
          ...
          this.$store.dispatch('LmsEquipClassRole/proTreeKeyword', this.key).then(() => {
            $.fn.zTree.init($("#purTree"), setting, this.$store.state.LmsEquipClassRole.treeList);
            treeObj = $.fn.zTree.getZTreeObj("purTree");
            // treeObj.expandAll(true);
            var nodes = treeObj.getNodes();
