<template>
  <div>
    ...
    <!-- 添加、编辑 -->
    <LmsEquipRepairEdit ref="editModal" @on-result-change="_search"></LmsEquipRepairEdit>
    <!-- 查看详情 -->
    <LmsEquipRepairDetail ref="detailModal"></LmsEquipRepairDetail>
  </div>
</template>
<script>
  import LmsEquipRepairEdit from './LmsEquipRepairEdit.vue'
  import LmsEquipRepairDetail from './LmsEquipRepairDetail.vue'
  ...
  export default {
    ...
    data() {
      return {
        btn: [
          {type: 'success', id: 'equip-repair-add', name: '添加'},
          {type: 'error', id: 'equip-repair-batch-delete', name: '删除'},
        ],
        btnObj: { //按钮权限
          edit: 'equip-repair-edit',
          delete: 'equip-repair-delete',
        },
        panelVal: '',
        heightSearch: '',
        loading: true,
        ...
        pageColumns: [
          ...
          {
            title: '计划名称', key: 'name', width: 160, align: 'center', fixed: 'left', ellipsis: true,
            render: (h, data) => {
              return h('div', [
                h('a', {
                  on: {
    ...
    methods: {
      ...
      _page() {
        this.$store.dispatch('LmsEquipRepair/page', this._searchParams()).then(() => {
          this.loading = false;
          this.getPage = this.$store.state.LmsEquipRepair.page;
        });
      },
      _pageChange(page) {
      ...
      _deleteByIds(ids, content) {
        this.$Modal.confirm({
          ...
          onOk: () => {
            this.$store.dispatch('LmsEquipRepair/deleteByIds', ids).then(() => {
      ...
      _detailModal(id) {
        // 查看
        this.$store.dispatch('LmsEquipRepair/getById', id).then(() => {
          this.$refs.detailModal._open(this.$store.state.LmsEquipRepair.model);
        });
      },
      _editModal(edit, id) {
        if (edit) {
          // 编辑
          this.$store.dispatch('LmsEquipRepair/getById', id).then(() => {
            this.$refs.editModal._open(this.$store.state.LmsEquipRepair.model);
          });
        } else {
          // 添加
