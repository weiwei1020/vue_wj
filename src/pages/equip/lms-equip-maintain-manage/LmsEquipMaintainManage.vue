<template>
  <div>
    ...
    <!-- 开始保养 -->
    <LmsEquipMaintainManageStart ref="startMaintainModal" @on-result-change="_search"></LmsEquipMaintainManageStart>
    <!-- 保养完成 -->
    <LmsEquipMaintainManageFinish ref="finishMaintainModal" @on-result-change="_search"></LmsEquipMaintainManageFinish>
    <!-- 查看详情 -->
    <LmsEquipMaintainManageDetail ref="detailModal"></LmsEquipMaintainManageDetail>
  </div>
</template>
<script>
  import LmsEquipMaintainManageStart from './LmsEquipMaintainManageStart.vue'
  import LmsEquipMaintainManageFinish from './LmsEquipMaintainManageFinish.vue'
  import LmsEquipMaintainManageDetail from './LmsEquipMaintainManageDetail.vue'
  ...
  export default {
    components: {
      LmsEquipMaintainManageStart,
      LmsEquipMaintainManageFinish,
      LmsEquipMaintainManageDetail,
      BtnList,
      BreadCrumbs,
      IconList
    },
    data() {
      return {
        btn: [
          {type: 'error', id: 'equip-maintain-manage-abolish', name: '废止'},
          {type: 'primary', id: 'equip-maintain-manage-submit', name: '提交'},
        ],
        applyLeftTime:'',
        applyRightTime:'',
        panelVal: '',
        ...
        pageColumns: [
          ...
          {title: '备注', key: 'remark', align: 'center', width: 160},
          {
            ...
            render: (h, data) => {
              let iconMsg = [];
              let startObj = {type: 'log-in', id: '', name: '开始保养'};
              let endObj = {type: 'checkmark', id: '', name: '保养完成'};
    ...
    methods: {
      ...
      _iconClick(res, data) {
        switch (res) {
          case '开始保养' :
            this.$refs.startMaintainModal._open(data.row);
            break;
          case '保养完成' :
            this.$refs.finishMaintainModal._open(data.row);
            break;
        }
      },
      _contHide() {
        ...
        if (3 === status) {
          for (let i = 0; i < dataList.length; i++) {
            if (dataList[i].status !== 2) {
              this.$Message.error('只能提交已保养的数据！');
              return;
            }
          }
          contents = "确定提交所选记录？";
          url = 'LmsEquipMaintainManage/submit';
        } else if (4 === status) {
          for (let i = 0; i < dataList.length; i++) {
            if (dataList[i].status !== 0) {
              this.$Message.error('只能废止待保养数据！');
              return;
            }
          }
          contents = "确定废止所选记录？";
          url = 'LmsEquipMaintainManage/abolish';
        } else {
