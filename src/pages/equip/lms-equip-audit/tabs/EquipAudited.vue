    ...
    methods: {
      ...
      _recall(id){
        this.$Modal.confirm({
          ...
          content : '确定撤回该记录？',
          onOk: () => {
            this.$store.dispatch('LmsEquipAudit/recallByGroupId', id).then(() => {
     ...
     /* _editModal(id) {
      ...
      _page() {
        this.$store.dispatch('LmsEquipAudit/auditPage', this._searchParams()).then(() => {
          this.loading = false;
          this.getPage = this.$store.state.LmsEquipAudit.page;
        });
      },
      _pageChange(page) {
      ...
      _detail(id) {
        // 查看
       /* this.$store.dispatch('LmsEquipAudit/getById', id).then(() => {
