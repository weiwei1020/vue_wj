<template>
  <div>
    <Modal v-model="showEditModal" :mask-closable="false" width="1100">
      ...
      <div slot="footer">
        <Button @click="_cancel" type="ghost" style="margin-left: 8px">确定</Button>
      <!--  <Button @click="_back"  type="danger">驳回审批</Button>
        <Button @click="_pass"  type="success">通过审批</Button>-->
      </div>
    </Modal>
    <!-- 通过原因 -->
    <LmsEquipOrderApproveEdit ref="approveModal" @on-result-change="_search"></LmsEquipOrderApproveEdit>
    <!-- 驳回原因 -->
    <LmsEquipOrderDelEdit ref="delModal" @on-result-change="_search"></LmsEquipOrderDelEdit>
  </div>
</template>
<script>
  /**
  ...
  import LmsEquipOrderApproveEdit from './LmsEquipOrderApproveEdit'
  import LmsEquipOrderDelEdit from './LmsEquipOrderDelEdit'
  ...
  export default {
    components: {
      BtnList,
      PageTable,
      LmsEquipOrderApproveEdit,
      LmsEquipOrderDelEdit,
    },
    data() {
      return {
        tableHeight: '300',
        ...
        loading:false,
        pageColumns: [
          {title: '仪器名称', key: 'name', width: 250, ellipsis: true},
          {
          ...
          {title: '申请原因', key: 'applyReason',ellipsis: true,},
        ],
        showModal: false,
        applyValue: '',
        getPage: {},
    ...
    methods: {
      ...
      _pass() {
          this.$refs.approveModal._open(this.groupId);
          this.showEditModal = false;
          this.$emit("on-result-change");
      },
      _back() {
          this.$refs.delModal._open(this.groupId);
          this.showEditModal = false;
          this.$emit("on-result-change");
      },
    }
