...
<script>
  ...
  export default {
        ...
        pageColumns: [
         ...
         /* {
          ...
          {title: '实验室编号', key: 'labNum', width: 160, align: 'center', ellipsis: true,},
          {
            ...
            render: (h, params) => {
              let useDate = '';
              if (params.row.useDate != undefined) {
                useDate = dateFormat(params.row.useDate, "yyyy-mm-dd HH:MM:ss")
              }
              return h('div', useDate);
            }
          },
          {
            ...
            render: (h, params) => {
              let useStartTime = '';
              if (params.row.useStartTime != undefined) {
                useStartTime = dateFormat(params.row.useStartTime, "yyyy-mm-dd HH:MM:ss")
              }
              return h('div', useStartTime);
            }
          },
          {
            ...
            render: (h, params) => {
              let useEndTime = '';
              if (params.row.useEndTime != undefined) {
                useEndTime = dateFormat(params.row.useEndTime, "yyyy-mm-dd HH:MM:ss")
              }
              return h('div', useEndTime);
            }
          },
    ...
    methods: {
      ...
      _getDate(date) {
        this.useTime.queryUseStartTime = date[0];
        this.useTime.queryUseEndTime = date[1];
      },
      _page() {
      ...
      _searchParams() {
        ...
        this.$extend(data, this.useTime, {status: 1});
